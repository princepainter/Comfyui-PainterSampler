...
def common_ksampler(model, seed, steps, cfg, sampler_name, scheduler, positive, negative, latent, denoise=1.0, disable_noise=False, start_step=None, last_step=None, force_full_denoise=True, noise_mask=None, callback=None, disable_pbar=False):
...
class PainterSampler:
    """
    Dual-Model Tandem Sampler: 100% replicates the generation effect of the official KSamplerAdvanced, with only dual-model input integrated.
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "return_leftover_noise": (["disable", "enable"], {"default": "disable"}),  # ←改名
            }
        }
    ...
    def sample(self, high_model, low_model, add_noise, noise_seed, steps, cfg,
        ...
        force_full_denoise = (return_leftover_noise == "disable")  # ←改名
        ...
        if start_at_step < switch_at_step:
            logger.info(f"Phase 1: High-noise [{start_at_step}→{switch_at_step}]")
            ...
            samples_stage1 = common_ksampler(
                ...
                noise_mask=latent_image.get("noise_mask", None), callback=callback, disable_pbar=disable_pbar
            )
        ...
        # 第二阶段：低噪声模型
        logger.info(f"Phase 2: Low-noise [{switch_at_step}→{end_at_step}]")
        samples_final = common_ksampler(
            ...
            noise_mask=current_latent.get("noise_mask", None), callback=callback, disable_pbar=disable_pbar
        )
        
        return (samples_final,)

NODE_CLASS_MAPPINGS = {"PainterSampler": PainterSampler}
NODE_DISPLAY_NAME_MAPPINGS = {"PainterSampler": "Painter Sampler"}
